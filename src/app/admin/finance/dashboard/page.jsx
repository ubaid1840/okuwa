...
export default function Page() {
  ...
  async function handleFilterResults() {
    ...
    axios
      .get(`/api/newroutes/healthcare/${UserState.value.data?.centerid}/finance?startdate=${startOfDay.getTime()}&enddate=${endOfDay.getTime()}`)
      .then((response) => {
        ...
        const temp = response.data?.map((item) => {
          return {
            ...
            [t("patientCreatedBy")]: `${
              item?.patientCreatedBy?.firstname || ""
            } ${item?.patientCreatedBy?.lastname || ""}`,
            [t("dob")]: item?.patient_dob
              ? moment(new Date(Number(item?.patient_dob))).format("DD/MM/YYYY")
              : "",
        ...
        <Wrap align={"flex-end"}>
          <WrapItem>
            <VStack align={"flex-start"} gap={0}>
              ...
              >
                <Calendar
                  disabled={!UserState.value.data?.centerid}
                  dateFormat="dd/mm/yy"
          ...
          <WrapItem>
            <Button
              isDisabled={!UserState.value.data?.centerid}
              width="100px"
              isLoading={loading}
              onClick={() => {
                setData();
...
const FinancialDistributionChart = ({ financialDistribution }) => {
  ...
  if (financialDistribution && financialDistribution.length > 0)
    return <ClusterColumn id={"clustercolumn-2"} data={[temp]} label={label} customColor={colorMode === 'light'?  "#000000" : "#FFFFFF"}/>;
};
